...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    if (tone > 1.5) {
        col = mix(col, vec3(tone), clamp((tone - 1.5) * 2.0, 0.0, 1.0));
    }
    col += dith;
...
export const LaserFlow: React.FC<LaserFlowProps> = ({
  ...
  useEffect(() => {
    ...
    const setSizeNow = () => {
      const w = mount.clientWidth || (typeof window !== 'undefined' ? window.innerWidth : 1000);
      const h = mount.clientHeight || (typeof window !== 'undefined' ? window.innerHeight : 800);
