...
const GridDistortion: React.FC<GridDistortionProps> = ({
  ...
  // Intersection Observer to only load WebGL when visible
  useEffect(() => {
  ...
  useEffect(() => {
    if (!webglSupported || !isInView || !containerRef.current) return;
    ...
    try {
      ...
      console.warn('GridDistortion: WebGL rendering context failed initialization.', e);
    ...
    textureLoader.load(
      ...
      undefined,
      (err) => {
        console.warn('GridDistortion: Failed to load texture.', err);
      }
    );
  ...
  }, [grid, mouse, strength, relaxation, imageSrc, webglSupported, isInView]);

  const showFallback = !webglSupported || !isInView;
  ...
  return (
    <div
      ...
      style={{
        ...
        minWidth: '0',
        minHeight: '0',
        position: isAbsoluteOrFixed ? undefined : 'relative',
        overflow: 'hidden'
      }}
    >
      {showFallback && (
